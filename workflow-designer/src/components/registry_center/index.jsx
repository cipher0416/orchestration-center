import {Bot, Box, Cpu, Database, Globe, Network, ShieldCheck, Zap, Search, LayoutDashboard} from "lucide-react";
...
const ICONS = [<Network size={22}/>, <Zap size={22}/>, <ShieldCheck size={22}/>, <Cpu size={22}/>, <Box size={22}/>,
    <Bot size={22}/>, <Database size={22}/>, <Globe size={22}/>];
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const rawObj = response || {};
                const enhancedData = Object.entries(rawObj).map(([key, val]) => {
                ...
                <div className={"flex flex-col gap-6 h-full min-h-0"}>
                    <div
                        ...
                        <div className={"relative"}>
                            <input type={"text"} placeholder={t('registry.search_placeholder')}
            ...
            <div className={"flex-1 min-w-0 flex flex-col p-2 min-h-0"}>
                {!agent ? (
                    <div
                        ...
                        {loading ? "Synchronizing..." : "Slect an agent"}
                    </div>
                ) : (<div
                    ...
                    <div
                        ...
                        <div className={"flex items-center gap-5"}>
                            <div
                                className={`p-4 rounded-2xl text-white shadow-lg ${agent.theme === 'emeralg' ? 'bg-emerald-500 shadow-emerald-500/20' : agent.theme === 'blue' ? 'bg-blue-600 shadow-blue-600/20' : 'bg-orange-600 shadow-orange-600/20'}`}>
                            ...
                            <div>
                                <div className={"flex items-center gap-3"}>
                                    ...
                                    <div className={"flex gap-2"}>
                                        <span
                                            className={"px-2 py-0.5 rounded bg-blue-100 dark:bg-blue-900/30 text-[10px] font-black text-blue-600"}>
                                            V{agent.version}
                                        </span>
                                        <button
                                ...
                                <div className={"flex gap-3 mt-2 items-center"}>
                                    ...
                                    <div className={"w-1 h-1 rounded-r-full bg-zinc-300"}/>
                                    <span className={"text-[10px] font-black text-zinc-400 uppercase"}>Protocol:</span>
                                    <span
                                        className={"text-[10px] font-black text-zinc-900 dark:text-zinc-200 uppercase"}>{agent.protocolVersion}</span>
                                </div>
                            </div>
                        </div>
                        <button onClick={() => setViewMode(viewMode === 'structured' ? 'raw' : 'structured')}
                                className={`flex items-center gap-2 px-3 py-1.5 rounded-md transition-all uppercase text-[12px] font-bold shadow-sm active:scale-95 active:shadow-inner ${isDark ? 'bg-blue-600 hover:bg-blue-500 text-white shadow-blue-900/20' : 'bg-blue-600 hover:bg-blue-700 text-white shadow-blue-200'}`}>
